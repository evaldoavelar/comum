...
begin
  objJson := TJSONObject.ParseJSONValue(jsonStr);
  context := TRttiContext.Create;
  tipo := context.GetType(aObj.ClassType);
  try
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //  var
         // propObj := GetValue(PPByte(tvArray.GetReferenceToRawData)^ + rDynArray.elementType.TypeSize * I, rDynArray.elementType).AsObject;

          TList<TObject>(obj).Add(subObj);
        end;
...
//function TJSONUtil.DeserializeDynArray(aTypeInfo: PTypeInfo; AObject: TObject; const aJsonArray: TJSONArray): TValue;
//begin
...
function TJSONUtil.GetValue(aAddr: Pointer; aType: TRttiType): TValue;
begin
  TValue.Make(aAddr, aType.Handle, Result);
end;

function TJSONUtil.GetValue(aAddr: Pointer; aTypeInfo: PTypeInfo): TValue;
begin
...
begin
  ...
  begin
    ...
    if (Field.Name = 'FListHelper') and
      ...
      (Field.FieldType.GetFields[1].Name = 'FCount') then
    begin
      hasTListHelper := True;
      Break;
    end
